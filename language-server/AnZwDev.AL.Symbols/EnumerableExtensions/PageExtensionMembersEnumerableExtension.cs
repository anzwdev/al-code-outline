...
{
    public static class PageExtensionMembersEnumerableExtension
    {

        public static IEnumerable<PageControlSymbol> GetAllControls(this PageExtensionSymbol pageExtensionSymbol, HashSet<PageControlKind>? controlKindFilter = null)
        {
            return GetAllControls(pageExtensionSymbol.ControlChanges);
        }

        public static IEnumerable<PageControlSymbol> GetAllControls(this List<PageControlChangeSymbol>? controlChanges, HashSet<PageControlKind>? controlKindFilter = null)
        {
            if ((controlChanges != null) && (controlChanges.Count > 0))
            {
                var stack = new Stack<EnumerableStackItem<PageControlSymbol>>();

                // process control changes
                for (int i = 0; i < controlChanges.Count; i++)
                {
                    var controls = controlChanges[i].Controls;
        ...
        public static IEnumerable<PageActionSymbol> GetAllActions(this PageExtensionSymbol pageExtensionSymbol, HashSet<PageActionKind>? actionKindFilter = null)
        {
            return GetAllActions(pageExtensionSymbol.ActionChanges, actionKindFilter);
        }

        public static IEnumerable<PageActionSymbol> GetAllActions(this List<PageActionChangeSymbol>? actionChanges, HashSet<PageActionKind>? actionKindFilter = null)
        {
            if ((actionChanges != null) && (actionChanges.Count > 0))
            {
                var stack = new Stack<EnumerableStackItem<PageActionSymbol>>();

                // process control changes
                for (int i = 0; i < actionChanges.Count; i++)
                {
                    var actions = actionChanges[i].Actions;
