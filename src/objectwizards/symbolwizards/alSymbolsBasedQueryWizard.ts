...
export class ALSymbolsBasedQueryWizard extends ALSymbolsBasedWizard {
    ...
    async showMultiQueryWizard(tableSymbols: AZSymbolInformation[]) {
        ...
        let startObjectId: number = await this.getObjectId(relativeFileDir, "query", `Please enter a starting ID for the query objects.`, 0);
        if (startObjectId < 0) {
            return;
        }
    ...
    async showQueryWizard(tableSymbol : AZSymbolInformation) {
        ...
        let objectId : number = await this.getObjectId(relativeFileDir, "query", "Please enter an ID for the query object.", 0);
        if (objectId < 0) {
            return;
        }
